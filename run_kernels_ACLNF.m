function [SW_V_kernels] = run_kernels_ACLNF(swperiods,R_or_L,ph_or_gr,ID,ifanis,ifdelete,ifplot,ifverbose)
% [SWV_kernels] = run_kernels_ACLNF(model,swperiods,R_or_L,ph_or_gr,ID,ifdelete,ifplot,ifverbose)
% 
...
ikernelfiles = writeKERNELCALCexecfile_ACLNF(swperiods,R_or_L(1),ph_gr,execfile_k,stripfile,eigfile,qmod,tabfile,qfile,kernelfile,ID,logfile);
...
for ip = 1:length(ikernelfiles)
    SW_V_kernels{ip,iv} = readMINEOS_kernelfile_ACLNF(ikernelfiles{ip,vees(iv)},R_or_L,phgropt(vees(iv)));
